...
const getDoseStatus = (medicine, time, minutesNow = getMinutesNow(), targetDate = new Date()) => {
...
const getInitialMedicines = () => {
  ...
  try {
    const parsedMedicines = savedMedicines ? JSON.parse(savedMedicines) : DEMO_MEDS
    const validMeds = Array.isArray(parsedMedicines) ? parsedMedicines : DEMO_MEDS
    return validMeds
      .map(normalizeMedicine)
      .filter(Boolean)
      .map(normalizeInitialStatus)
  } catch {
    return DEMO_MEDS.map(normalizeMedicine).filter(Boolean).map(normalizeInitialStatus)
  }
}
...
function Dashboard() {
    ...
    const checkReminders = () => {
      ...
      setMedicineList((currentMedicines) => {
        ...
        const nextMedicines = currentMedicines.map((medicine) => {
          if (!isMedicineScheduledOnDate(medicine, new Date())) return medicine;
          ...
          medicine.scheduleTimes.forEach(time => {
            ...
            if (["Due Now", "Due Soon", "Delayed"].includes(doseStatus)) {
              ...
              if (isDueWindow && medCopy.notificationSentFor !== `${todayKey}-${time}`) {
                notifyDoseDue(medicine, time)
                didChange = true
                medChanged = true
                medCopy.notificationSentFor = `${todayKey}-${time}`
              }
            } else if (doseStatus === "Missed" && !(medCopy.adherenceHistory || []).some(h => h.date === todayKey && h.time === time)) {
              didChange = true
              medChanged = true
  ...
  const filteredMedicines = activeMedicines.filter((medicine) =>
    ...
    const doses = rTimes.map(t => ({ time: t, status: getDoseStatus(med, t) }))
    const activeDose =
  ...
  const priorityDose = useMemo(() => {
    ...
    const candidates = medicineList
      ...
      .flatMap(m => m.scheduleTimes.map(time => ({
        ...
        status: getDoseStatus(m, time, minutesNow)
      })))
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const dayMeds = medicineList
        ...
        .flatMap(m => m.scheduleTimes.map(time => ({
          ...
          name: m.name,
          dosage: m.dosage,
          status: getDoseStatus(m, time, minutesNow, date),
  ...
  const adherenceStats = useMemo(() => {
    ...
    activeList.forEach(med => {
      ...
      med.scheduleTimes.forEach(t => {
        ...
        if (isMedicineScheduledOnDate(med, new Date())) {
          total++
        }
        const s = getDoseStatus(med, t)
  ...
  return (
    <>
      ...
      <header className="mb-4 flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/[0.055] p-4 shadow-2xl shadow-slate-950/20 backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between">
        <div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <label className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-slate-300 shadow-lg shadow-slate-950/10 transition focus-within:border-emerald-300/35 focus-within:bg-emerald-400/10 sm:w-72">
            <svg
              viewBox="0 0 24 24"
          ...
          <button
            ...
            className={`relative grid h-12 w-12 place-items-center rounded-2xl border shadow-lg transition duration-300 hover:-translate-y-0.5 ${
              notificationPermission === "denied" 
                ? "border-rose-500/30 bg-rose-500/5 text-rose-400" 
                : "border-white/10 bg-black/25 text-emerald-100 hover:border-emerald-300/35 hover:bg-emerald-400/10"
            }`}
          >
            <span className={`absolute right-3 top-3 h-2 w-2 rounded-full transition-all duration-500 ${
              notificationPermission === "granted" ? "bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,0.5)]" :
              notificationPermission === "denied" ? "bg-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.5)]" :
            ...
            >
              {notificationPermission === "denied" ? (
                <path d="M18 8A6 6 0 0 0 6 8c0 7-3 7-3 7h18s-3 0-3-7M2 2l20 20M13.73 21a2 2 0 0 1-3.46 0" />
              ) : (
                <path d="M18 9a6 6 0 0 0-12 0c0 7-3 7-3 7h18s-3 0-3-7Zm-4.3 10a2 2 0 0 1-3.4 0" />
              )}
            </svg>
          </button>

          <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/25 px-3 py-2 shadow-lg shadow-slate-950/10">
      ...
      <section
        id="overview"
        className="relative overflow-hidden rounded-[2.5rem] border border-emerald-200/15 bg-white/[0.06] p-6 shadow-2xl shadow-slate-950/30 backdrop-blur-xl transition duration-500 hover:border-emerald-300/30 hover:bg-white/[0.08] sm:p-8 lg:p-10"
      >
        <div className="absolute -right-10 -top-16 h-48 w-48 rounded-full bg-emerald-500/15 blur-3xl pointer-events-none" />
        <div className="absolute -bottom-14 left-10 h-36 w-36 rounded-full bg-teal-500/10 blur-2xl pointer-events-none" />
        ...
        <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex-1">
            ...
            <div className="mt-6 flex flex-wrap gap-x-6 gap-y-4">
              ...
              <div className="flex items-center gap-3">
                <span className={`flex h-10 w-10 items-center justify-center rounded-xl ${adherenceStats.missed > 0 ? 'bg-amber-400/10 text-amber-400' : 'bg-slate-400/10 text-slate-500'}`}>
              ...
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-500/10 text-orange-400 shadow-lg shadow-orange-950/20">
                  🔥
                </span>
                <div>
              ...
              {adherenceStats.lowStockCount > 0 && (
                <div className="flex items-center gap-3 rounded-2xl bg-rose-500/10 px-4 py-2 ring-1 ring-rose-500/20">
            ...
            {priorityDose && (
              <div className="mt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl bg-amber-400/5 p-5 border border-amber-500/20 group hover:border-amber-500/40 transition duration-300">
                ...
                <button onClick={() => markMedicineTaken(priorityDose.medicine.id)} className="rounded-xl bg-amber-500 px-4 py-2 text-xs font-black text-black transition hover:bg-amber-400">Mark Taken</button>
              </div>
            )}
          </div>

          <div className="flex flex-col items-center justify-center gap-2 rounded-[2rem] bg-black/40 p-8 shadow-inner ring-1 ring-white/10 lg:w-56">
        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:flex">
            <button
              onClick={() => setIsModalOpen(true)}
              className="rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-emerald-950/35 transition duration-300 hover:-translate-y-0.5 hover:from-emerald-400 hover:to-teal-400 hover:shadow-emerald-500/20"
            >
              Add medicine
            </button>
            <button
              onClick={() => setIsCalendarOpen(true)}
              className="rounded-xl border border-white/10 bg-black/25 px-4 py-3 text-center text-sm font-bold text-slate-200 shadow-lg shadow-slate-950/10 transition duration-300 hover:-translate-y-0.5 hover:border-emerald-300/30 hover:bg-emerald-400/10 hover:text-white"
            >
              Calendar
            </button>
          </div>
      ...
      {useMemo(() => medicineList.filter(m => !m.archived && getStockStatus(m.stock, m.dosesPerDay).status !== "Healthy"), [medicineList]).length > 0 && (
        <section className="mt-8 animate-[modalIn_0.5s_ease-out]">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {medicineList.filter(m => !m.archived).map(med => {
